<!--　企業一覧のページ　 created by Hirokazu Niimoto　←（゜Д゜)ﾊｧ?　-->
...
try{
  ...
  while ($row = $stmt->fetch(PDO::FETCH_ASSOC)){
    ...
    if(strpos($pagelay,$row['InternshipName'])){
      continue;
    }

    $enterpriseinfo = $row['EnterpriseInfo'];
    $businessinfo = $row['BusinessInfo'];
    if (mb_strlen($enterpriseinfo)>=90){
      $enterpriseinfo = mb_substr($row['EnterpriseInfo'],0,90)."...";
    }
    if (mb_strlen($businessinfo)>=90){
      $businessinfo = mb_substr($row['BusinessInfo'],0,90)."...";
    }

    $enterprisename = $row['InternshipName'];

    $pagelay .= <<<_PAGE
    <div class="col-md-6 col-12 pt-4">
      <div class="card card-image" style="background-image: url({$row['InternshipImage']});background-size: cover;">

        <!-- Content -->
        <div class="text-white text-center d-flex align-items-center rgba-black-strong py-5 px-4">
          <div>
            <!--<h5 class="orange-text"><i class="fas fa-desktop"></i> Software</h5>  indigo-->

            <h3 class="card-title pt-2 text-center"><strong>{$enterprisename}</strong></h3>
            <p class="mt-5" style="height:120px;"><strong>企業情報：{$enterpriseinfo}</strong></p>
            <p class="mt-1" style="height:120px;"><strong>事業情報：{$businessinfo}</strong></p>
            <a class="btn btn-deep-orange waves-effect waves-light" data-toggle="modal" onclick="enterprisematterlist('{$enterprisename}')" data-target="#enterprisematterlistmodal""><i class="fas fa-clone left"></i>詳細を見る</a>
          </div>
        </div>
 ...
 <html lang="ja" dir="ltr">
   <head>
    ...
    .enterprisematter:hover {
        background-color:#98FB98;
        padding: 25px;
        font-size: 24px;
        }
        /*マウスオーバーしたらメッセージ表示*/
     .enterprisematter:hover  .rightarrow {
       display:block;
       margin-top: 12px;
     }
  ...
  <body>
    ...
    <!--企業案件一覧モーダル-->
    <div class="modal fade" id="enterprisematterlistmodal" tabindex="-1" role="dialog" aria-labelledby=""
      aria-hidden="true">
      <div class="modal-dialog modal-dialog-scrollable modal-dialog-centered" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h4 class="modal-title" id="exampleModalLongTitle"><strong><i class="far fa-copy mr-3 ml-3"></i>募集一覧</strong></h4>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
              <span aria-hidden="true">&times;</span>
            </button>
          </div>
          <div class="modal-body" id="enterprisematterlist">
            <div class="card mt-1">
              <div class="card-body">
                <a class="" onclick="messagedisplay(s)" data-toggle="modal" data-target="#bookmarkmodal" data-dismiss="modal"><h4 class="card-title">株式会社re-vol</h4></a>
              </div>
            </div>
          </div>
          <div class="modal-footer">
    ...
    <p class="mt-3"></p>
    <div class="pt-5">
      <h1 class="text-center mt-5" style="background:linear-gradient(transparent 80%, #64a3e1 0%);"><span>企業一覧</span></h1>
    </div>


    <div class="row pt-3 ml-md-5 mr-md-5 ml-1 mr-1">
      <?php echo $pagelay ?>
    </div>
    ...
    <footer class="page-footer font-small mt-4 footerinfo" style="background-color:black;">
      ...
      <div class="row mt-4">
        ...
        <div class="col-md-3" style="font-size:15px;">
          <p class="feature-title font-bold mt-1 text-md-left text-center"><i class="fas fa-info fa-x mr-4" aria-hidden="true"></i><a href="https://re-vol.net/">企業情ページ</a></p>
